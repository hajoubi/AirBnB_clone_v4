#!/usr/bin/python3
'''Module docstring: Provides routes for managing
the relationship between places and amenities in the API.
This module includes endpoints
for retrieving, adding, updating, and removing amenities
linked to a place.
'''
...
def amenity_by_place(place_id):
    """
    handler for fetching amenities associated with the place
    fetch the amenities linked to place by its ID, if not exist
    """
...
def unlink_amenity_from_place(place_id, amenity_id):
    """
    handler method for unlinking amenity and place
    If either the place or the amenity does not exist,
...
def link_amenity_to_place(place_id, amenity_id):
    """
    handler method for linking amenity and place
    If either the place or the amenity does not exist,
    a  404 error is returned.
    If the amenity is already linked to the place,
    the existing amenity details are returned.
        :param place_id: place id
        :param amenity_id: amenity id
