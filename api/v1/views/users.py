#!/usr/bin/python3
"""module docstring: manages the users view in the API
The module handles the retrieval, creation, updating,
...
def get_users(user_id=None):
    ...
    If a user ID is provided, returns the user with that ID.
    Otherwise, returns all users.
    Omits sensitive attributes like 'places' and 'reviews' from the response.
...
def add_user():
    """Adds a new user with the data from the request body
    Expects the request data to be in JSON format with at least an
...
def update_user(user_id):
    ...
    Only updates fields not in the exclusion list.
    Raises a BadRequest exception if the request data is not in JSON format.
    Raises a NotFound exception if the user ID does not exist.
    """
